...
mp_pool_t * mp_create_pool(size_t size)
{
    ...
    ret = posix_memalign((void **)&p, MP_ALIGNMENT, size + sizeof(mp_pool_t) + sizeof(mp_node_t));
    if (ret) {
        return NULL;
    }
...
static void * mp_alloc_block(mp_pool_t *pool, size_t size)
{
    ...
    ret = posix_memalign((void **)&m, MP_ALIGNMENT, psize);
    if (ret) {
        return NULL;
    }
...
// free all large chunck
void mp_free_all(mp_pool_t *pool)
{
    mp_large_t  *l;
